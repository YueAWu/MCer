...
#         super(MCerLSTM, self).__init__(*args, **kwargs)
#         self.positional_size = positional_size
#
#         self.arg_self_attention = SelfAttention(self.embedding_size, n_heads[0], self.dropout)
#         self.layer_norm_a = nn.LayerNorm(self.embedding_size)
...
#         self.event_self_attention = SelfAttention(self.hidden_size, n_heads[1], self.dropout)
#         self.layer_norm_e = nn.LayerNorm(self.hidden_size)
#         self.linear = InitLinear(self.hidden_size*2, 1, dis_func="normal", func_value=0.02)
#
#     def adjust_event_chain_embedding(self, embedding):
#         """
#         shape: (batch_size, 52, embedding_size) -> (batch_size * 5, 36, embedding_size)
